...
def get_split_dataloaders(
    batch_size: int,
    downsample: int,
    num_anthropic_samples: int,
    train_ratio: float = 0.8,
    random_seed: int = 42,
) -> tuple[torch.utils.data.DataLoader, torch.utils.data.DataLoader]:
    anthropic_df = load_anthropic_datasets(dataset_type="explicit")
    train_pairs = contrastive_gender_pairs(anthropic_df)[:num_anthropic_samples]
...
def get_mean_pooled_adapter_loss(
    model: AutoModelForCausalLM,
    batch: tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, Any],
    layer: int,
    adapter: LinearProjection,
    ...
    a_attention_mask_BL = attention_mask_BL[0::2]
    b_attention_mask_BL = attention_mask_BL[1::2]

    a_tokens_bD = einops.reduce(
        a_tokens_bLD * a_attention_mask_BL[:, :, None], "b l d -> b l", "sum"
    )
    b_tokens_bD = einops.reduce(
        b_tokens_bLD * b_attention_mask_BL[:, :, None], "b l d -> b l", "sum"
    )

    mse_B = torch.nn.functional.mse_loss(
        a_tokens_bD, b_tokens_bD, reduction="none"
    ).mean(dim=-1)

    mse = mse_B.float().mean()

    cosine_sim_B = torch.nn.functional.cosine_similarity(
        a_tokens_bD, b_tokens_bD, dim=-1
    )
    cosine_sim = cosine_sim_B.float().mean()
...
def train_adapter(
    ...
    for epoch in range(num_epochs):
        for i, batch in enumerate(train_dataloader):
            loss, similarity_loss, cosine_sim, regularization_loss = (
                get_mean_pooled_adapter_loss(model, batch, layer, adapter, lambda_reg)
            )
