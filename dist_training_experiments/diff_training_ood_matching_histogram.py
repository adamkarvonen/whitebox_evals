...
def get_matching_length_pairs(
    contrastive_pairs: list[str], tokenizer: AutoTokenizer
) -> list[str]:
    new_pairs = []

    for i in range(0, len(contrastive_pairs), 2):
        a_toks = tokenizer.encode(contrastive_pairs[i])
        b_toks = tokenizer.encode(contrastive_pairs[i + 1])
        if len(a_toks) == len(b_toks):
            new_pairs.append(contrastive_pairs[i])
            new_pairs.append(contrastive_pairs[i + 1])

    print(
        f"Found {len(new_pairs)} matching length pairs out of {len(contrastive_pairs)} contrastive pairs"
    )
    return new_pairs
...
def get_split_dataloaders(
    ...
    anthropic_df = load_anthropic_datasets(dataset_type="explicit")
    train_pairs = contrastive_gender_pairs(anthropic_df)
    train_pairs = get_matching_length_pairs(train_pairs, tokenizer)
...
def get_adapter_loss(
    ...
    if include_mse_bl:
        return loss, similarity_loss, cosine_sim, regularization_loss, mse_BL
    else:
...
def get_adapter_val_loss(
    ...
    for batch in dataloader:
        loss, sim_loss, cos_sim, reg_loss, mse_bl = get_adapter_loss(
            model, batch, layer, adapter, lambda_reg, include_mse_bl=True
        )
        total_loss += loss.item()
        total_sim_loss += sim_loss.item()
        total_cos_sim += cos_sim.item()
        total_reg_loss += reg_loss.item()
        mse_bl_list.append(mse_bl)
        n_batches += 1
    ...
    if mse_bl_filename is not None:
        with open(mse_bl_filename, "wb") as f:
            pickle.dump(mse_bl_list, f)
...
def train_adapter(
    ...
    with torch.no_grad():
        val_loss, val_similarity_loss, val_cosine_sim, val_regularization_loss = (
            get_adapter_val_loss(
                model,
                test_dataloader,
                layer,
                adapter,
                lambda_reg,
                mse_bl_filename="initial_mse_bl.pkl",
            )
        )
        print(
        ...
        with torch.no_grad():
            val_loss, val_similarity_loss, val_cosine_sim, val_regularization_loss = (
                get_adapter_val_loss(
                    ...
                    adapter,
                    lambda_reg,
                    mse_bl_filename=f"mse_bl_epoch_{epoch}.pkl",
                )
            )
            print(
...
if __name__ == "__main__":
    ...
    # train_dataloader, test_dataloader = get_split_dataloaders(
